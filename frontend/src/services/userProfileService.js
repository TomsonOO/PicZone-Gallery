...
export async function updateUserAvatar(token, file) {
    ...
    const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/user/update/avatar`, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${token}`,
        },
...
export async function getProfileImage(profileImageId) {
    ...
    const presignedUrlRes = await fetch(`${backendUrl}/api/images/presigned-url/${profileImageData.objectKey}`);
    if (!presignedUrlRes.ok) {
        const errData = await presignedUrlRes.json().catch(() => ({}));
        throw new Error(errData.message || `Failed to fetch presigned URL for ${profileImageData.objectKey}`);
    }
