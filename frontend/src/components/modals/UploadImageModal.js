...
export default function UploadImageModal({ isOpen, onRequestClose }) {
  ...
  return (
    ...
    >
      <div
        className={`bg-[#e0e0e0] dark:bg-gradient-to-b dark:from-[#111f4a] dark:to-[#1a327e] p-6 rounded shadow-lg max-w-5xl w-full flex flex-col md:flex-row md:space-x-6 relative transform-gpu transition-transform duration-300 ${opacity ? 'translate-y-0' : '-translate-y-10'}`}
      >
        <button
          onClick={handleClose}
        ...
        <form
          className='w-full md:w-2/5 flex flex-col space-y-6 mt-10'
