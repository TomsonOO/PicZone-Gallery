...
export class GetBookInfoQueryHandler
  implements IQueryHandler<GetBookInfoQuery, string>
{
  ...
  async execute(query: GetBookInfoQuery): Promise<string> {
    this.logger.log(
      `Processing book information query for "${query.title}" by ${query.author}: ${query.query}`,
    );
    ...
    try {
      ...
      const answer = await this.bookAnalysisPort.getBookInformation(
        context,
        query.query,
      );
      return answer;
      ...
      this.logger.error(
        `Failed to get book information: ${error.message}`,
        error.stack,
      );
